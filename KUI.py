...
class KakuroUI(Frame):
    ...
    def draw_grid(self):
        # Draw vertical and horizontal lines to create the grid
        for i in range(10):
            stretch = 0 if i % 9 else 1
        ...
        # Create rectangles to represent the grid cells
        for i in range(9):
            for j in range(9):
                if [i, j] not in self.game.data_fills:
                    self.canvas.create_rectangle(
                        MARGIN + j * SIDE + 1, MARGIN + i * SIDE + 1,
